...
import { collection, onSnapshot, addDoc, setDoc, doc, getDocs, updateDoc, deleteDoc } from 'firebase/firestore';
import { db, auth, FIREBASE_API_KEY } from '../firebase';
import { signOut } from 'firebase/auth';
import { LogOut, Database, Plus, Shield, Store, Users, Copy, Check, UserPlus, Building, Loader2, KeyRound, RefreshCw, Trash2, Pencil, Save, X } from 'lucide-react';
...
const SuperAdminDashboard = () => {
  ...
  const [editingOutlet, setEditingOutlet] = useState(null); // { id, name, slug, address, ownerUid }
                        ...
                        <button
                          onClick={async () => {
                            ...
                            try {
                              ...
                              pushToast('Пользователь удалён', 'success');
                            } catch (e) { pushToast('Ошибка: ' + e.message, 'error'); }
                          }}
          ...
          <Card className="border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-slate-100">
              {outlets.length === 0 ? (
                ...
                const isEditing = editingOutlet?.id === o.id;
                return (
                  <div key={o.id} className="p-5 lg:px-8 hover:bg-slate-50 transition-colors">
                    {isEditing ? (
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={async () => {
                              try {
                                await updateDoc(doc(db, 'outlets', o.id), {
                                  name: editingOutlet.name,
                                  slug: editingOutlet.slug,
                                  address: editingOutlet.address || '',
                                  ownerUid: editingOutlet.ownerUid
                                });
                                setEditingOutlet(null);
                                pushToast('Точка обновлена', 'success');
                              } catch (e) { pushToast('Ошибка: ' + e.message, 'error'); }
                            }}
                            className="flex items-center gap-1.5 px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-bold hover:bg-blue-500 transition-all"
                          >
                            <Save size={14} /> Сохранить
                          </button>
                          <button onClick={() => setEditingOutlet(null)} className="flex items-center gap-1.5 px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-xs font-bold hover:bg-slate-200 transition-all">
                            <X size={14} /> Отмена
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <button title="Перегенерировать ключ" className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-slate-700 transition-all"
                            onClick={async () => {
                              if (!window.confirm('Перегенерировать ключ? Старый перестанет работать.')) return;
                              try { await updateDoc(doc(db, 'outlets', o.id), { connectCode: generateConnectCode() }); pushToast('Ключ обновлён', 'success'); } catch (e) { pushToast('Ошибка: ' + e.message, 'error'); }
                            }}
                          ><RefreshCw size={15} /></button>
                          <button title="Редактировать" className="p-1.5 hover:bg-blue-50 rounded-lg text-slate-400 hover:text-blue-600 transition-all"
                            onClick={() => setEditingOutlet({ id: o.id, name: o.name || '', slug: o.slug || '', address: o.address || '', ownerUid: o.ownerUid || '' })}
                          ><Pencil size={15} /></button>
                          <button title="Удалить точку" className="p-1.5 hover:bg-red-50 rounded-lg text-slate-300 hover:text-red-500 transition-all"
                            onClick={async () => {
                              if (!window.confirm(`Удалить точку "${o.name}"? Это необратимо.`)) return;
                              try { await deleteDoc(doc(db, 'outlets', o.id)); pushToast('Точка удалена', 'success'); } catch (e) { pushToast('Ошибка: ' + e.message, 'error'); }
                            }}
